...
int main()
{
    ...
    ME::Shader shader2 = ME::Shader::create("resources/shaders/testV.glsl", "resources/shaders/testF.glsl");
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * 3 * size, faces, GL_STATIC_DRAW);  // Number of every vertex.
    ...
    loop = [&]
    {
        ...
        shader2.setUnifMat4("viewMat", camera.getViewMatC());
        shader2.setUnifMat4("projMat", camera.getProjMatC());
        ...
        shader2.setUnifVec3("color", { 0.7f, 0.3f, 0.3f });
        glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
        glDrawElements(GL_TRIANGLES, 3 * (int)size, GL_UNSIGNED_INT, 0);

        shader2.setUnifVec3("color", { 0.0f, 0.0f, 0.0f });
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
        glDrawElements(GL_TRIANGLES, 3 * (int)size, GL_UNSIGNED_INT, 0);

        glBindBuffer(GL_ARRAY_BUFFER, 100);
