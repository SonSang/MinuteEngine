...
vec4 pmSingle();        // Use single layer sampling to determine new texture coordinates
vec4 pmMultiple();      // Use multi layer sampling to determine new texture coordinates
vec4 pmOcclusion();     // Use multi layer sampling and linear interpolation to determine new texture coordinates
/* ============================================================================================= */
...
vec3 renderByMaterial() {
    ...
    vec3 diffuse = vec3(0.0, 0.0, 0.0);
    for (int i = 0; i < lightNum; i++)
        diffuse += phongLightDiffuse(material.diffuse, normal, light[i]);
...
void main(void) {
    eyePosition = (oModelViewMat * oPosition).xyz;
    eyeNormal = normalize((oModelViewMat * vec4(oNormal.xyz, 0.0)).xyz);
    computeTBN();

    if(polygonMode) {
        if (textureMode)
            gl_FragColor = vec4(renderByTexture(), 1.0);
        else {
            if (phongMode)
                gl_FragColor = vec4(renderByMaterial(), 1.0);
            else
...
float inShadowFactor(Shadow shadow) {
    ...
    vec4 shadowMapCoords4 = shadow.shadowMat * oModelMat * oPosition;
...
vec4 pmSingle() {
    ...
    return oTexCoord - vec4(p.xy, 0.0, 0.0);
}
vec4 pmMultiple() {
    vec3 tangentSpaceViewDir;
    mat3 invEyeTBN;
    ...
    return vec4(cTexCoord.xy, 0.0, 0.0);
}
vec4 pmOcclusion() {
    vec3 tangentSpaceViewDir;
    mat3 invEyeTBN;
    ...
    return vec4(fTexCoord.xy, 0.0, 0.0);
}
/* ============================================================================================= */